...
if __name__ == "__main__":
    ...
    network = Network(
        ...
        optimizer = Adam(momentum = 0.9, beta_constant = 0.99, weight_decay=1e-6), 
        # optimizer = RMSProp(beta_constant = 0.9),
        # optimizer = Momentum(momentum=0.9),
        scheduler = StepLR(initial_learning_rate=0.0004, decay_rate=0.5, decay_interval=55), 
    ...
    for idx, cost in enumerate(network.fit(generator=generator, batch_size=batch_size, accumulate=accumulate, epochs = 2000000000, gradient_transformer=AutoClipper(10))):
