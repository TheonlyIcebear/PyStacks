...
def get_anchor_data(grid_size, bboxes):
    ...
    dimensions = dimensions[dimensions[:, 0] < 0.4] 
    dimensions = dimensions[dimensions[:, 1] < 0.4]
...
if __name__ == "__main__":
    ...
    image_width, image_height = [608, 608]
    yolo_head_count = 3
    ...
    bias_initializer = YoloSplit(presence_initializer=Fill(np.log(0.01)), xy_initializer=Fill(0), dimensions_initializer=Fill(np.log(0.75)), class_initializer=Fill(0), classes=classes, anchors=3)
    ...
    network = Network(
        ...
        optimizer = Adam(momentum = 0.9, beta_constant = 0.99, weight_decay=0), 
        # optimizer = RMSProp(beta_constant = 0.9),
        # optimizer = Momentum(momentum=0.9),
        scheduler = StepLR(initial_learning_rate=0.0001, decay_rate=0.5, decay_interval=75), 
