...
class ClipGradient:
    ...
    def _clip_gradient(self, gradient):
        for idx, layer in enumerate(gradient):
            if isinstance(layer, cp.ndarray):
                ...
                gradient[idx] = gradient[idx].astype(dtype)
    ...
    def _get_norm(self, gradient):
        ...
        l2sum_safe = cp.where(cp.isnan(l2sum), cp.ones_like(l2sum), l2sum)
        l2sum_safe = cp.where(cp.isinf(l2sum_safe), cp.ones_like(l2sum), l2sum_safe)

        l2norm = cp.sqrt(l2sum_safe)
        return l2norm.astype(gradient.dtype)
...
class AutoClipper:
    ...
    def _get_total_norm(self, gradient):
        ...
        return cp.clip(self._get_norm(norms.flatten()), 0, 1000)
    ...
    def forward(self, gradient):
        ...
        print(total_norm, clip_norm.dtype)
